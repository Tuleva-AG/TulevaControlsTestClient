...
import { HeaderColumn, IBaseEntity, TulevaPage } from "./lib";
...
function TestPage() {
  ...
  useEffect(() => {
    let items: ITestItem[] = [
      ...
      },
      {
        ...
        subject: "Wert 4",
        description:
          "D - Lorem ipsum dolor, sit amet consectetur adipisicing elit. Doloremque, recusandae neque! Corporis distinctio atque natus ea molestias in doloribus, ratione eius, tenetur aliquam saepe aspernatur dicta rem voluptas error nam.",
      },
      {
        id: "5",
        subject: "Wert 5",
        description:
          "F - Lorem ipsum dolor, sit amet consectetur adipisicing elit. Doloremque, recusandae neque! Corporis distinctio atque natus ea molestias in doloribus, ratione eius, tenetur aliquam saepe aspernatur dicta rem voluptas error nam.",
      },
      {
        id: "6",
        subject: "Wert 6",
        description:
          "C - Lorem ipsum dolor, sit amet consectetur adipisicing elit. Doloremque, recusandae neque! Corporis distinctio atque natus ea molestias in doloribus, ratione eius, tenetur aliquam saepe aspernatur dicta rem voluptas error nam.",
      },
    ];
  ...
  const onDelete: (id: string | undefined) => Promise<void> = async (
    id: string | undefined
  ): Promise<void> => {
    if (id) {
      setItemsState([
        ...itemsState.filter((item: ITestItem): boolean => item.id !== id),
      ]);
    }
  };
  ...
  const onExit: (testItem: ITestItem) => void = (testItem: ITestItem) => {
    let items: ITestItem[] = itemsState;
    setItemsState([
      ...items.filter((item: ITestItem): boolean => item.id !== testItem.id),
    ]);
  };
  ...
  const renderViewer: (item: ITestItem) => JSX.Element = (
    item: ITestItem
  ): JSX.Element => {
    return (
      <>
  ...
  const renderEditor: (item: ITestItem) => JSX.Element = (
    item: ITestItem
  ): JSX.Element => {
    return (
      <>
    ...
    return (
      <>
        <HeaderColumn
          ...
          sortState={sortState}
          onClick={(sortField: string, sortDescending: boolean) =>
            handleSortClick(sortField, sortDescending)
          }
        >
          {"Subject"}
        </HeaderColumn>
